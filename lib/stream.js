...
THE SIX VOICES:
intel — 3-4 short sentences of forensic connective tissue: a date, a named person, a mechanism, a disputed claim, an unexpected adjacency across two domains. SPECIFICS ONLY — things that could in principle be checked. If a sentence would survive with the seed swapped for another word, delete it.
  ACCURACY IS NOT OPTIONAL HERE. This voice claims the authority of fact, so a fabricated fact discredits the whole piece. Never invent an etymology, a root word, a date, or a attribution. If you are not confident, choose a different fact you ARE confident of, or say plainly that the claim is disputed. A true ordinary fact beats a false impressive one. Do not use technical terms (like "perfect number") unless you are certain of their actual definition.
  End with a fragment listing the threads found.
illusionist — exactly five framings. Each ONE sentence. THE TEST THAT MATTERS: if framing 2 is true, framing 1 must be FALSE. Not five associations, not five aspects, not five things it "represents" — five claims that CANNOT COEXIST. Five compatible readings is the failure mode and it is the most common one. Study the SHAPE of this example, then write nothing like it — it uses a different seed on purpose. For the seed "a staircase": WEAK, because all can be true at once — "it means ascent / it means transition / it connects two worlds." STRONG, because each destroys the others — "it is the only honest architecture, because it admits you are not yet where you are going" vs "it is a machine for humiliating the body, and we call that humiliation aspiration" vs "it is not a passage at all; nobody has ever lived on a staircase, which is the only fact about it that matters." NEVER reuse the wording of any example — if your framing echoes an example's phrasing, you have failed. Commit to each framing absolutely, as if the other four were wrong.
...
• heretic MUST be placed after at least two other voices. It prosecutes what has already been said — it cannot indict a voice that has not spoken yet. If heretic appears before intel, illusionist or echo in your order, the arrangement is wrong. Never place it first or second.
...
• DECIDE THE SILENCE FIRST, before anything else. Ask: which voice would this seed be BETTER WITHOUT? Concrete triggers — a seed that is abstract or has no real history (a number, a shape, a bare concept) usually leaves intel with nothing but trivia: silence it. A seed already drowning in received meaning (a famous symbol, a cliché) does not need illusionist to add more: silence it. A seed that is plain and concrete has nothing to dissolve: silence guardian. A seed nobody has inflated has nothing to prosecute: silence heretic.
  An empty withhold list should be RARE — reserve it for seeds that genuinely need every voice. A performance that always uses all six is not an arrangement, it is a checklist.
...
function cleanScore(raw) {
  ...
  // The heretic prosecutes what has already been said. If the model put it
  // first or second, it can only predict — so move it to third at the earliest.
  const hi = order.indexOf('heretic');
  if (hi > -1 && hi < 2 && order.length >= 3) {
    order.splice(hi, 1);
    order.splice(Math.min(2, order.length), 0, 'heretic');
  }
